...
class FrozenCLIPImageEmbedder(AbstractEncoder):
    ...
    def forward(self, image):
        ...
        gz = self.mapper(gz)
        z = self.final_ln(gz)  # b,257,1024
        # get the outputs from shallow layer
        # lz = all_hidden[self.shallow_index][:,1:] # b,256,1024
        # z  = torch.cat([gz, lz], dim=1)
        return z
