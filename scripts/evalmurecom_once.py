...
def generate_multifg(fg_list, mask_list):
    fg_img_list = []
    for idx, fg_mask in enumerate(mask_list):
...
def generate_murecom_batch(bg_img, fg_img, bbox, fg_mask):
    ...
    y_bottom=min(y_bottom,H-1) 
    fg_bbox = [x_left, y_top, x_right, y_bottom]
    # img = fg_img[fg_bbox[1]:fg_bbox[3], fg_bbox[0]:fg_bbox[2]]
    fg_img, fg_bbox = rescale_image_with_bbox(fg_img, fg_bbox)
    fg_img = np.array(fg_img)
    fg_mask = fg_mask.resize((fg_img.shape[1], fg_img.shape[0]))
    fg_mask = np.array(fg_mask)
    fg_img, fg_mask, fg_bbox = crop_foreground_by_bbox(fg_img, fg_mask, fg_bbox)
...
def prepare_input(batch, model, shape, device, num_samples):
    ...
    fg_tensor = batch['fg_img'].to(device)  # (1, 3, 224, 224)

    c = model.get_learned_conditioning(
        fg_tensor)  # clip encoder (1, 257, 1024)
    c = model.proj_out(c)  # (1, 257, 768)
    uc = model.learnable_vector.repeat(c.shape[0], c.shape[1], 1)  # 1,1,768
    return test_model_kwargs, c, uc

def prepare_multifg_input(batch, model, shape, device, num_samples):
    if num_samples > 1:
        for k in batch.keys():
            if isinstance(batch[k], torch.Tensor):
                batch[k] = torch.cat([batch[k]] * num_samples, dim=0)
...
def argument_parse():
    ...
    parser.add_argument(
        ...
        default="/data/chenjiaxuan/project/DreamCom-Image-Composition/MureCom/"
    )
    parser.add_argument(
        "--test_time",
        type=str,
    ...
    parser.add_argument(
        "--ckpt_dir",
        default="/data/chenjiaxuan/project/ObjectStitch_train/ObjectStitch_train/experiments/objectstitch/2024-06-06T09-54-35_car/checkpoints",
...
if __name__ == "__main__":
    opt = argument_parse()
    # weight_path = os.path.join(opt.ckpt_dir, "last.ckpt")
    weight_path = opt.ckpt_dir
    # weight_path = os.path.join(opt.ckpt_dir,"ObjectStitch.pth")
    assert os.path.exists(weight_path), weight_path
    config = OmegaConf.load(opt.config)
    clip_path = '/data/chenjiaxuan/project/ObjectStitch-Image-Composition/ControlCom-Image-Composition/checkpoints/openai-clip-vit-large-patch14'
    ...
    num_samples = opt.num_samples  # default : 1
    guidance_scale = opt.scale
    if opt.fixed_code:
        seed_everything(opt.seed)
    start_code = torch.randn(
        [num_samples]+shape, device=device)  # [1, 4, 64, 64]
    test_dir = os.path.join(opt.testdir, opt.package_name)
    outdir = os.path.join(opt.outdir,opt.package_name,opt.fg_name,f'testtime_{opt.test_time}')
    os.makedirs(outdir, exist_ok=True)
    bg_root = os.path.join(test_dir, 'bg')
    fg_root = os.path.join(test_dir, opt.fg_name)
    bg_list = []
    fg_list = []
    mask_list = []
    for ij in range(20):
        bg_path = os.path.join(bg_root,str(ij) + '.jpg')
        bg_img = Image.open(bg_path).convert('RGB')
        bg_list.append(bg_img)
    for ii in range(5):
        fg_path = os.path.join(fg_root,str(ii) + '.jpg')
        fg_img = Image.open(fg_path).convert('RGB')
        fg_list.append(fg_img)
        mask_path = os.path.join(fg_root,str(ii) + '.png')
        mask = Image.open(mask_path).convert('RGB')
        mask_list.append(mask)
    file_path = os.path.join(bg_root, 'bbox.txt')
    bboxes = []
    ...
    fg_img_list = generate_multifg(fg_list, mask_list)
    for ii, bg_img in enumerate(bg_list):
        for j in range(5):
            start = time.time()
            batch = generate_murecom_batch(bg_img, fg_list[j], bboxes[ii], mask_list[j])
            # batch = generate_murecom_multifg_batch()
            batch['fg_img_list'] = fg_img_list
            test_model_kwargs, c, uc = prepare_multifg_input(
            ...
            # for i in range(comp_img.shape[0]):
                # if i > 0:
                #     res_path = os.path.join(outdir, str(ii) + f'_sample{i}.png')
                # else:
                #     res_path = os.path.join(
                #         outdir, str(j) +'sample'+ str(ii) + '.jpg')
                # res_path = os.path.join(outdir, 'bg_'+ str(ii) +'fg_' + str(j) + 'sample_' + str(i) + f'.jpg')
                # save_image(comp_img[i], res_path)
                # print('save result to {}'.format(res_path))
            if not opt.skip_grid:
                grid_img = generate_image_grid(batch, comp_img)
                grid_path = os.path.join(
                    outdir, str(ii) +'sample'+ str(j) + f'_grid.jpg')
